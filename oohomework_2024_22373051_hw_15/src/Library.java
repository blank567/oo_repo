...
public class Library {
    ...
    @SendMessage(from = "Library", to = "Borrower")
    ...
    public void borrowBook(LibraryCommand libraryCommand, LibraryRequest libraryRequest) {
        ...
        if (bookShelf.getBookNum(bookId) > 0 || bookDriftCorner.getBookNum(bookId) > 0) {
            if (!borrower.isHaveEnoughCreditScore()) {
    ...
    public void orderBook(LibraryCommand libraryCommand, LibraryRequest libraryRequest) {
        ...
        Borrower borrower = borrowers.get(id);
        if (bookId.isTypeA() || !borrower.isHaveEnoughCreditScore()) {
            LibrarySystem.PRINTER.reject(libraryCommand);
        } else if (bookId.isTypeB()) {
            if (borrower.isContainB() || isHaveBOrder(id)) {
                LibrarySystem.PRINTER.reject(libraryCommand);
            } else {
            ...
            if (borrower.isContainBook(bookId) || isHaveCOrder(bookId, id)) {
                LibrarySystem.PRINTER.reject(libraryCommand);
            } else {
                orders.add(libraryCommand);
                LibrarySystem.PRINTER.accept(libraryCommand);
    ...
    public void open(LocalDate data) {
        for (String id : borrowers.keySet()) {
            borrowers.get(id).check(data);
        }
        ...
        while (iterator.hasNext()) {
            ...
            if (ChronoUnit.DAYS.between(command.getDate(), data) >= 5) {
                ...
                Borrower borrower = borrowers.get(request.getStudentId());
                borrower.addCreditScore(-3);
        ...
        for (LibraryBookId bookId : borrowReturnOffice.getBooks()) {
            if (bookId.isTypeBU() || bookId.isTypeCU()) {
                if (bookDriftCorner.getTimesOfBook(bookId) >= 2) {
                    borrowers.get(bookDriftCorner.getDonor(bookId)).addCreditScore(2);
                    ...
                    bookDriftCorner.addBook(bookId, bookDriftCorner.getDonor(bookId));
    ...
    public void donateBook(LibraryCommand libraryCommand, LibraryRequest libraryRequest) {
        ...
        borrowers.get(id).addCreditScore(2);
        bookDriftCorner.addBook(libraryRequest.getBookId(), id);
        LibrarySystem.PRINTER.accept(libraryCommand);
    }
    ...
    public boolean isHaveCOrder(LibraryBookId libraryBookId, String id) {
        for (LibraryCommand command : orders) {
            LibraryRequest request = ((LibraryReqCmd) command).getRequest();
            if (request.getBookId().equals(libraryBookId) && request.getStudentId().equals(id)) {
                return true;
            }
        }
        for (LibraryCommand command : appointment.getOrders()) {
    ...
    public boolean isHaveBOrder(String id) {
        for (LibraryCommand command : orders) {
            LibraryRequest request = ((LibraryReqCmd) command).getRequest();
            if (request.getBookId().isTypeB() && request.getStudentId().equals(id)) {
                return true;
            }
        }
        for (LibraryCommand command : appointment.getOrders()) {
    ...
    public void renewBook(LibraryCommand libraryCommand, LibraryRequest libraryRequest) {
        ...
        String id = libraryRequest.getStudentId();
        Borrower borrower = borrowers.get(id);
        if (bookId.isTypeBU() || bookId.isTypeCU() || !borrower.isHaveEnoughCreditScore()) {
            LibrarySystem.PRINTER.reject(libraryCommand);
            return;
        }
    ...
    public void queryCreditScore(LocalDate date, String id) {
        if (!borrowers.containsKey(id)) {
            borrowers.put(id, new Borrower(id));
        }
        LibrarySystem.PRINTER.info(date, id, borrowers.get(id).getCreditScore());
    }
}
